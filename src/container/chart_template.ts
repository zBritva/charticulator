...
export class ChartTemplate {
  ...
  public instantiate(
    ...
    // Perform inferences
    const chartInferences = async (chart, template) => {
      for (const inference of template) {
        ...
        if (inference.nestedChart) {
          ...
          const specification = (object as IObject<NestedChartElementProperties>).properties.specification as Specification.Chart;
          chartInferences(specification, this.template.inference);
        }
      }
    }
    chartInferences(chart, this.template.inference);
    return {
      chart,
      defaultAttributes: this.template.defaultAttributes,
    };
